...
try{
  ...
  const userId=existing?.id??(await storage.createLocalUser(email,password,process.env.ADMIN_FIRST_NAME?.trim()||"Ardenvia",process.env.ADMIN_LAST_NAME?.trim()||"Administrator")).id;
