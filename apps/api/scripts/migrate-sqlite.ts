...
const hasTable=(table:string)=>Boolean(source.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(table));

try{
  const sourceCounts:Record<string,number>={};
  const tableNames=["users","local_credentials","accounts",...(hasTable("internal_transfers")?["internal_transfers"]:[]),...(hasTable("peer_transfers")?["peer_transfers"]:[]),"transactions","cards","beneficiaries","cash_flow","user_preferences"];
  ...
  await target.begin(async(sql)=>{
    ...
    for(const row of rows("accounts")) await sql`INSERT INTO accounts (id,user_id,name,type,account_number,masked_number,currency,balance_cents,available_balance_cents,interest_rate_bps) VALUES (${String(row.id)},${String(row.user_id)},${String(row.name)},${String(row.type)},${row.account_number===undefined||row.account_number===null?null:String(row.account_number)},${String(row.masked_number)},${String(row.currency)},${Number(row.balance_cents)},${Number(row.available_balance_cents)},${row.interest_rate_bps===null?null:Number(row.interest_rate_bps)}) ON CONFLICT (id) DO UPDATE SET user_id=excluded.user_id,name=excluded.name,type=excluded.type,account_number=coalesce(accounts.account_number,excluded.account_number),masked_number=CASE WHEN accounts.account_number IS NOT NULL THEN accounts.masked_number ELSE excluded.masked_number END,currency=excluded.currency,balance_cents=excluded.balance_cents,available_balance_cents=excluded.available_balance_cents,interest_rate_bps=excluded.interest_rate_bps`;
    if(hasTable("internal_transfers"))for(const row of rows("internal_transfers"))await sql`INSERT INTO internal_transfers (id,user_id,source_account_id,destination_account_id,amount_cents,note,reference,status,created_at) VALUES (${String(row.id)},${String(row.user_id)},${String(row.source_account_id)},${String(row.destination_account_id)},${Number(row.amount_cents)},${String(row.note)},${String(row.reference)},${String(row.status)},${String(row.created_at)}) ON CONFLICT (id) DO NOTHING`;
    if(hasTable("peer_transfers"))for(const row of rows("peer_transfers"))await sql`INSERT INTO peer_transfers (id,sender_user_id,source_account_id,recipient_user_id,destination_account_id,amount_cents,note,reference,status,created_at) VALUES (${String(row.id)},${String(row.sender_user_id)},${String(row.source_account_id)},${String(row.recipient_user_id)},${String(row.destination_account_id)},${Number(row.amount_cents)},${String(row.note)},${String(row.reference)},${String(row.status)},${String(row.created_at)}) ON CONFLICT (id) DO NOTHING`;
    for(const row of rows("transactions")) await sql`INSERT INTO transactions (id,account_id,internal_transfer_id,peer_transfer_id,description,merchant,category,amount_cents,direction,status,reference,created_at) VALUES (${String(row.id)},${String(row.account_id)},${row.internal_transfer_id===undefined||row.internal_transfer_id===null?null:String(row.internal_transfer_id)},${row.peer_transfer_id===undefined||row.peer_transfer_id===null?null:String(row.peer_transfer_id)},${String(row.description)},${String(row.merchant)},${String(row.category)},${Number(row.amount_cents)},${String(row.direction)},${String(row.status)},${String(row.reference)},${String(row.created_at)}) ON CONFLICT (id) DO UPDATE SET account_id=excluded.account_id,internal_transfer_id=coalesce(transactions.internal_transfer_id,excluded.internal_transfer_id),peer_transfer_id=coalesce(transactions.peer_transfer_id,excluded.peer_transfer_id),description=excluded.description,merchant=excluded.merchant,category=excluded.category,amount_cents=excluded.amount_cents,direction=excluded.direction,status=excluded.status,reference=excluded.reference,created_at=excluded.created_at`;
