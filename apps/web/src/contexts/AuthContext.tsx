...
async function authRequest(path:string,options?:RequestInit){
  ...
  if(!response.ok){const error=await response.json().catch(()=>({message:response.statusText}));throw new Error(error.message??"Authentication failed");}
...
export function AuthProvider({children}:{children:React.ReactNode}){
  ...
  return <AuthContext.Provider value={{user,signIn,signUp,signOut,isAuthenticated:Boolean(user),isLoading}}>{children}</AuthContext.Provider>;
}
