    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.72fr]">
        ...
        <aside className="space-y-6">
          <section className="rounded-2xl bg-[#14251f] p-6 text-[#f4f7f5] sm:p-7"><LockKeyhole className="size-5 text-white/70" /><h2 className="mt-8 font-semibold">Security</h2><p className="mt-2 text-sm leading-relaxed text-white/60">Use a unique password with at least 12 characters.</p><div className="mt-5 space-y-3"><Input type="password" autoComplete="current-password" placeholder="Current password" value={currentPassword} onChange={(event)=>setCurrentPassword(event.target.value)} /><Input type="password" autoComplete="new-password" placeholder="New password" value={newPassword} onChange={(event)=>setNewPassword(event.target.value)} /><Input type="password" autoComplete="new-password" placeholder="Confirm new password" value={confirmPassword} onChange={(event)=>setConfirmPassword(event.target.value)} /></div><Button variant="secondary" className="mt-5 w-full" disabled={currentPassword.length<8||newPassword.length<12||newPassword!==confirmPassword||passwordMutation.isPending} onClick={()=>passwordMutation.mutate({currentPassword,newPassword})}>{passwordMutation.isPending?"Changing…":"Change password"}</Button></section>
