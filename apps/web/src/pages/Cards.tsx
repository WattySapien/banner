...
export default function Cards() {
  ...
  const [page,setPage]=useState(1); const pageSize=4;
  const updateCard = useMutation({
    ...
    onSuccess: (card) => {
      ...
      toast({ title: card.status === "frozen" ? "Card frozen" : "Card ready to use", description: `${card.network} ending in ${card.lastFour} was updated.` });
    },
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {isLoading ? <div className="grid gap-5 lg:grid-cols-[repeat(2,minmax(0,25rem))]">{[0, 1].map((item) => <div key={item} className="h-72 max-w-[25rem] animate-pulse rounded-2xl bg-muted" />)}</div> : cards.length === 0 ? <section className="grid min-h-72 place-items-center rounded-2xl bg-muted/55 p-8 text-center"><div><CreditCard className="mx-auto size-6 text-muted-foreground"/><h2 className="mt-4 font-semibold">No cards issued</h2><p className="mt-1 text-sm text-muted-foreground">Issued physical and virtual cards will appear here.</p></div></section> : <><section className="grid gap-5 lg:grid-cols-[repeat(2,minmax(0,25rem))]" aria-label="Payment cards">{cards.slice((page-1)*pageSize,page*pageSize).map((card,index)=><SecurePaymentCard key={card.id} card={card} index={index} isUpdating={updateCard.isPending} onToggleStatus={()=>updateCard.mutate({id:card.id,update:{status:card.status==="active"?"frozen":"active"}})}/>)}</section><ListPagination page={page} pageSize={pageSize} total={cards.length} onPageChange={setPage}/></>}
