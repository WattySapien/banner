  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-sm text-muted-foreground">Money movement</p><h1 className="mt-1 text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">Activity</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Search and review transactions across your accounts.</p></div>
        <Button variant="outline" onClick={exportCsv} disabled={filtered.length === 0}><Download className="mr-2 size-4" />Export CSV</Button>
      </header>
      ...
      <section className="rounded-2xl border bg-card p-5 sm:p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2" aria-label="Filter activity">
            {(["all", "credit", "debit", "pending"] as ActivityFilter[]).map((value) => <Button key={value} size="sm" variant={filter === value ? "default" : "outline"} onClick={() => setFilter(value)} className="capitalize">{value === "credit" ? "Money in" : value === "debit" ? "Money out" : value}</Button>)}
          </div>
        </div>
